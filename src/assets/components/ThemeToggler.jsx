...
export function ThemeProvider({ children }) {
  ...
  const values = { theme, toggleTheme, isDark: theme === "dark" };
  return (
...
function ThemedButton({ children, variant = "primary", onClick }) {
  ...
  return (
    <button
      onClick={onClick}
      className={`px-4 py-2 rounded-md transition ${getButtonClasses()}`}
    >
      {children}
    </button>
  );
    ...
    <div
      className={`p-4 rounded-md ${
        isDark
          ...
          : "border-l-5 border-orange-500 bg-orange-50 text-gray-800"
      }`}
    >
